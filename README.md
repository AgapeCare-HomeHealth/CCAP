...
## Temporary Web Mock Mode

`CCAP.Web/appsettings.json` contains:
...
When enabled, the Web UI uses local mock services/data and does not require `CCAP.API` for login, dashboard, patients, users, roles/permissions, or patient workflow data.

Set `MockData:Enabled` to `false` to restore API-backed behavior. The API URL remains configured under `Api:BaseUrl`.

The mock layer is implemented behind the existing Web service classes so page components do not need to change when switching back to the API.
